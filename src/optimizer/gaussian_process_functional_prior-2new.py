...
def scale_posterior(mu_posterior, sigma_posterior, mu_est, sigma_est, mu_source, sigma_source):
   
    w_posterior = 1 / sigma_posterior
    w_prior = 1 / sigma_est 
    w_source = 1 / sigma_source

    total_weight = w_posterior + w_prior + w_source

   
    w_posterior /= total_weight
    w_prior /= total_weight
    w_source /= total_weight

   
    mean = w_posterior * mu_posterior + w_prior * mu_est + w_source * mu_source

   
    sigma = torch.sqrt(w_posterior**2 * sigma_posterior**2 + w_prior**2 * sigma_est**2 + w_source**2 * sigma_source**2)
...
class ShiftedExpectedImprovement(ExpectedImprovement):
    ...
    def __init__(
        ...
        source_gp_model_path = "~/Quantile/src/experiments/GPmodels/m4-Daily_gp_model_gaussian.pth"

        
        self.source_gp_model= self.load_source_gp(source_gp_model_path)
    ...
    def forward(self, X: Tensor) -> Tensor:
        ...
        with torch.no_grad():
            ...
            source_posterior = self.source_gp_model.posterior(X)
            mu_source = source_posterior.mean
            sigma_source = source_posterior.variance.clamp_min(1e-6).sqrt()
    ...
    def load_source_gp(self, model_path: str):
        ...
        dummy_train_X = torch.randn(2, 2)  # Assuming 2 input dimensions, change if needed
        dummy_train_Y = torch.randn(2, 1)  # Assuming 1 output dimension, change if needed
